...
Layout:
  src/data/daily_log.csv          one row per ticker per day — the tracking database
  watchlist/_daily/YYYY-MM-DD.md  human-readable daily brief (inside the knowledge base)
  weekly/YYYY-Www.md              weekly digest (same content that goes to Discord/email)
...
def write_daily_report(rows: list[dict]) -> object:
    ...
    lines = [
        f"# 🌷 Falling Angle Daily Brief — {date}",
        "",
        f"_{len(rows)} ticker(s) analyzed · raw log: `src/data/daily_log.csv`_",
        "",
    ]
    for row in rows:
...
def write_weekly_report(data: dict) -> object:
    ...
    lines = [f"# 🌷 Falling Angle Weekly Digest — {data['date_str']}", ""]
