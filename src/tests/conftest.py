"""
Shared fixtures for the Falling Angle test suite.
...
Ground rules (enforced here so every test stays offline and hermetic):
  - `_safe_env` (autouse) blanks every secret/network knob on the `config`
    module, so a local `.env` with real SMTP/Discord credentials can never
    leak into a test run.
  - `tmp_config` redirects every storage path (CSV, reports, pending,
    analysis, index.md, ledger.md) into the pytest tmp dir, so tests never
...
def _safe_env(monkeypatch):
    """No SMTP, no Discord — regardless of the developer's local .env."""
...
def tmp_config(tmp_path, monkeypatch):
    ...
    knowledge = tmp_path / "stock_knowledge"
    ...
    monkeypatch.setattr(config, "STOCK_KNOWLEDGE_DIR", knowledge)
    monkeypatch.setattr(config, "INDEX_PATH", knowledge / "index.md")
    monkeypatch.setattr(config, "LEDGER_PATH", knowledge / "ledger.md")
    return config
