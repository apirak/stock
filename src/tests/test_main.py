...
def _args(**overrides) -> argparse.Namespace:
    defaults = dict(mode="daily", tickers="", dry_run=False,
...
def test_resolve_tickers_arg_wins_over_everything(tmp_config):
    assert main._resolve_tickers(" nvda , msft ,") == ["NVDA", "MSFT"]
...
def test_resolve_tickers_reads_daily_input_file(tmp_config):
    ...
    assert main._resolve_tickers(None) == ["NVDA", "MSFT"]


def test_resolve_tickers_env_watchlist_beats_index(tmp_config, monkeypatch):
    monkeypatch.setattr(config, "WATCHLIST", ["DIS", "NKE"])
    monkeypatch.setattr(universe, "tickers", lambda **k: ["NVDA"])
    assert main._resolve_tickers("") == ["DIS", "NKE"]


def test_resolve_tickers_falls_back_to_index_universe(tmp_config, monkeypatch):
    monkeypatch.setattr(config, "WATCHLIST", [])
    monkeypatch.setattr(universe, "tickers",
                        lambda automated_only=False: ["NVDA", "MSFT", "GOOGL"])
    assert main._resolve_tickers(None) == ["NVDA", "MSFT", "GOOGL"]
...
def test_run_daily_writes_csv_report_and_pending_file(tmp_config,
    ...
    data_date = str(rows[0]["Date"])[:10]        # reports are named by the data date
    report = tmp_config.DAILY_REPORT_DIR / f"{data_date}.md"
    pending = tmp_config.PENDING_DIR / f"daily-{main._today()}.json"
    assert report.exists()
    assert pending.exists()

    payload = json.loads(pending.read_text(encoding="utf-8"))
    assert payload["date"] == main._today()
...
def test_finalize_daily_merges_analysis_into_csv_and_report(tmp_config, make_row,
    ...
    monkeypatch.setattr(main.discord_notifier, "send_daily_summary", lambda rows: False)
...
def test_finalize_weekly_dry_run_writes_preview_only(tmp_config, make_row, capsys):
    ...
    out = capsys.readouterr().out
    assert rc == 0
    assert "Falling Angle Weekly Digest" in out               # subject printed
