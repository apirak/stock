"""
Discord webhook delivery, in the Feb persona (charactor/feb/persona.md).
...
share a single 🧡 line, and every message closes with the decision disclaimer.
The webhook display name defaults to "Feb 🌷" (config.DISCORD_USERNAME).
...
watchlist/_daily/ and weekly/. Never raises; each send returns False on failure.
"""
...
DISCLAIMER = "⚠️ Feb เป็นผู้ช่วยวิเคราะห์นะคะ การตัดสินใจซื้อขายเป็นของเจ้าตัวเสมอ 🌷"
...
def _build_messages(data: dict) -> list[dict]:
    ...
    lead = [f"🌷 **จาก Watch list — Falling Angle Weekly {data['date_str']}** ค่ะ"]
    if picks:
        picks_line = " | ".join(
            f"{p['row']['Ticker']}: {p['row']['Moat Impairment Verdict']}" for p in picks
        )
        lead.append(f"⭐ หุ้นน่าสนใจสัปดาห์นี้: {_trunc(picks_line, 200)}")
    else:
        lead.append("สัปดาห์นี้ไม่มีตัวที่ผ่านเกณฑ์ Falling Angle ค่ะ — การรอคอยก็เป็น position หนึ่งนะคะ")
...
def build_daily_message(rows: list[dict]) -> tuple[str, list[dict]]:
    """Feb-style daily summary (pure — no network). Returns (content, embeds).

    Per charactor/feb/persona.md: interesting names (Pass verdicts) become one
    green embed each (ราคา / FV / MoS / มุมของ Feb); everyone else is a compact
    one-liner; names still priced above fair value share one 🧡 line; the
    decision disclaimer closes the message.
    """
    date = str(rows[0].get("Date", ""))[:10]
    interesting = [r for r in rows
                   if str(r.get("Moat Impairment Verdict", "")).startswith("Pass")]
    overpriced = [r for r in rows
                  if (d := _cell_float(r, "Discount (%)")) is not None
                  and d < config.MIN_DISCOUNT_FOR_BUY]

    lines = [f"🌷 **จาก Watch list — Daily {date}** ค่ะ"]
    if interesting:
        lines.append(f"สแกนครบ {len(rows)} ตัว วันนี้มีน่าสนใจ {len(interesting)} ตัวค่ะ ✨")
    else:
        lines.append(f"สแกนครบ {len(rows)} ตัวแล้ว แต่วันนี้ไม่มีอะไรน่าสนใจค่ะ — "
                     "ทุกตัวยังไม่ผ่านเกณฑ์ Falling Angle (ห้ามมั่งมีขึ้นมานะคะ)")
    for row in rows:
        if row in interesting:
            continue
        discount = _cell_float(row, "Discount (%)")
        discount_txt = f"ส่วนลด {discount:.1f}%" if discount is not None else "ส่วนลด n/a"
        lines.append(
            f"{_verdict_emoji(row.get('Moat Impairment Verdict', ''))} "
            f"**{row.get('Ticker', '')}** — {row.get('Moat Impairment Verdict', 'n/a')} → "
            f"{row.get('Strategic Action', 'n/a')} | {discount_txt} | "
            f"[📊 chart]({chart_urls(row.get('Ticker', ''))['TradingView']})"
        )
    if overpriced:
        names = ", ".join(
            f"{r.get('Ticker')} ({_cell_float(r, 'Discount (%)'):+.1f}%)" if _cell_float(r, "Discount (%)") is not None
            else f"{r.get('Ticker')}"
            for r in overpriced
        )
        lines.append(f"🧡 แพงกว่ามูลค่าแบบเงียบ ๆ: {names}")
    lines.append(f"-# {DISCLAIMER}")

    embeds = []
    for row in interesting[:3]:  # Discord allows 10; keep the daily message calm
        discount = _cell_float(row, "Discount (%)")
        embeds.append({
            "title": f"✅ {_trunc(row.get('Ticker', ''), 30)} — {_trunc(row.get('Company Name'), 80)}",
            "description": _trunc(row.get("Deep-Dive", ""), 900),
            "color": _verdict_color(row.get("Moat Impairment Verdict", "")),
            "fields": [
                {"name": "ราคา vs FV", "value": (
                    f"${_trunc(row.get('Market Price ($)'), 20)} vs "
                    f"${_trunc(row.get('Fair Value ($)'), 20)}"
                ), "inline": True},
                {"name": "MoS (ส่วนลด)", "value": _trunc(
                    f"{discount:.1%}" if discount is not None else "n/a", 40), "inline": True},
                {"name": "แผน", "value": _trunc(row.get("Strategic Action", ""), 60), "inline": True},
                {"name": "📈 ดูกราฟ", "value": _trunc(
                    chart_links_md(row.get("Ticker", "")), _MAX_FIELD_VALUE), "inline": False},
            ],
        })
    return _trunc("\n".join(lines), _MAX_CONTENT), embeds


def send_daily_summary(rows: list[dict]) -> bool:
    """Post today's Feb summary to the daily channel. Called by --mode finalize
    (daily) once final verdicts are merged."""
    if not config.discord_ready():
        print("[discord] DISCORD_WEBHOOK_URL not configured; daily summary skipped")
        return False
    if not rows:
        return False
    content, embeds = build_daily_message(rows)
    try:
        resp = requests.post(config.DISCORD_WEBHOOK_URL, json=_payload(content, embeds), timeout=30)
        resp.raise_for_status()
