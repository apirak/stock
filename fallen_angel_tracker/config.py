...
# Morningstar's wide-moat feed is a paid API, so the "scan" input is this
# curated universe (or the optional daily_input.txt / --tickers override).
# ---------------------------------------------------------------------------
WATCHLIST = [
    t.strip().upper()
    for t in os.getenv("WATCHLIST", "UNH,TGT,DIS,PYPL,SBUX,NKE,BA,ADM").split(",")
    if t.strip()
]

# Manual Morningstar-style fair values, e.g. "UNH=520,TGT=180".
...
# ---------------------------------------------------------------------------
# Analytical framework thresholds (see README / SETUP_GUIDE)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Rotating Buffett-style mini-lesson curriculum for the weekly email.
# The LLM expands the selected brief; without an LLM the brief itself is sent.
# ---------------------------------------------------------------------------
